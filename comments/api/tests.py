...
class CommentApiTests(TestCase):
    ...
    def test_create(self):
        # anonymous user can not comment
        response = self.anonymous_client.post(COMMENT_URL)
        self.assertEqual(response.status_code, 403)

        # you cannot comment without param
        response = self.linghu_client.post(COMMENT_URL)
        self.assertEqual(response.status_code, 400)

        # only tweet_id is not allowed
        response = self.linghu_client.post(COMMENT_URL, {'tweet_id' : self.tweet.id})
        self.assertEqual(response.status_code, 400)

        # only content is not allowed
        response = self.linghu_client.post(COMMENT_URL, {'content' : 'haha'})
        self.assertEqual(response.status_code, 400)

        # too long content is not allowed
        response = self.linghu_client.post(COMMENT_URL, {
            'content' : '1' * 141,
        })
        self.assertEqual(response.status_code, 400)
